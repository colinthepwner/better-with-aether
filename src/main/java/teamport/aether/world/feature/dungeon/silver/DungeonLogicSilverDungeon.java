...
public class DungeonLogicSilverDungeon extends DungeonLogic {
    ...
    protected boolean placeDungeon(World world, Random random) {
        return new WorldFeatureAetherSilverDungeon(Directions.horizontalIndex(direction)).generate(this, world, this.seed, position.getX(), position.getY(), position.getZ());
    }

    @Override
    protected boolean canPlaceDungeon(World world) {
        return new WorldFeatureAetherSilverDungeon(Directions.horizontalIndex(direction)).canPlace(world, position.getX(), position.getY(), position.getZ());
    }
