...
public class ScreenSignSkyrootEditor extends Screen {
    ...
    public void removed() {
        ...
        if (this.mc.currentWorld != null && this.mc.currentWorld.isClientSide) {
            ...
            if (enumsignpicture != null)
                this.mc.getSendQueue().addToSendQueue(new PacketSignUpdate(this.entitySign.tilePos.x, this.entitySign.tilePos.y, this.entitySign.tilePos.z, this.entitySign.signText, enumsignpicture.getId(), this.entitySign.getColor().id));
        }
    ...
    public void keyPressed(char eventCharacter, int eventKey, int mx, int my) {
        ...
        if ((NetCharacters.isAllowed(eventCharacter)) && this.entitySign.signText[this.editLine].length() < 15) {
